...
class NemwebTests(unittest.TestCase):
    def test_fetch_bytes_retries_after_403_and_eventually_succeeds(self) -> None:
        ...
        with (
            patch("aemo_forecast.nemweb.urllib.request.urlopen", side_effect=[forbidden, forbidden]) as mock_urlopen,
            patch("aemo_forecast.nemweb.subprocess.run", side_effect=[CalledProcessError(22, ["curl"]), MagicMock(stdout=b"ok")]) as mock_curl,
        ...
        self.assertEqual(2, mock_curl.call_count)
